...
def create_wsv(con):
    con.sql('''CREATE VIEW IF NOT EXISTS wsv AS
...
def insert_wsv(con):
    con.sql("insert or ignore into web_sales select distinct(*) from wsv")
...
if __name__ == "__main__":
    ...
    print(f'The Insertion for LF_WS started: {start_time}')
    create_wsv(con)
    insert_wsv(con)
    end_time = time.time()
    print(f'The time for the Insertion (LF_WS) took: {end_time-start_time} seconds')
