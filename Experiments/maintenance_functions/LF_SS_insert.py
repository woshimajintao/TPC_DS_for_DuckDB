...
def create_ssv(con):
    con.sql('''CREATE view IF NOT EXISTS ssv as
...
def insert_ssv(con):
    con.sql("insert or ignore into store_sales select distinct(*) from ssv")
...
if __name__ == "__main__":
    ...
    print(f'The Insertion for LF_SS started: {start_time}')
    create_ssv(con)
    insert_ssv(con)
    end_time = time.time()
    print(f'The time for the Insertion (LF_SS) took: {end_time-start_time} seconds')
