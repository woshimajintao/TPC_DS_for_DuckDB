...
def create_csv(con):
    con.sql('''CREATE view IF NOT EXISTS csv as
...
def insert_csv(con):
    con.sql("insert or ignore into catalog_sales select distinct(*) from csv")
...
if __name__ == "__main__":
    ...
    print(f'The Insertion for LF_CS started: {start_time}')
    create_csv(con)
    insert_csv(con)
    end_time = time.time()
    print(f'The time for the Insertion (LF_CS) took: {end_time-start_time} seconds')
