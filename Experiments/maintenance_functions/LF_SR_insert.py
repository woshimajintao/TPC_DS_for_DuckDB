...
def create_srv(con):
    con.sql('''CREATE view IF NOT EXISTS srv as
...
def insert_srv(con):
    con.sql("insert or ignore into store_returns select distinct(*) from srv")
...
if __name__ == "__main__":
    ...
    print(f'The Insertion for LF_SR started: {start_time}')
    create_srv(con)
    insert_srv(con)
    end_time = time.time()
    print(f'The time for the Insertion (LF_SR) took: {end_time-start_time} seconds')
